...
class CmsLayoutTest < ActiveSupport::TestCase
  ...
  def test_fixtures_validity
    Occams::Cms::Layout.all.each do |layout|
  ...
  def test_content_tokens
    layout = Occams::Cms::Layout.new(content: "a {{cms:text content}} b")
    expected = [
      "a ",
  ...
  def test_content_tokens_nested
    layout_a = Occams::Cms::Layout.new(content: "a {{cms:text content}} {{cms:text footer}} b")
  ...
  def test_content_tokens_nested_with_fragment_subclass_tag
    layout_a = Occams::Cms::Layout.new(content: "a {{cms:markdown content}} b")
    layout_b = Occams::Cms::Layout.new(content: "c {{cms:text content}} d")
    layout_b.parent = layout_a
    expected = [
      "a ",
  ...
  def test_content_tokens_nested_without_content_tag
    layout_a = Occams::Cms::Layout.new(content: "a {{cms:text footer}} b")
    layout_b = Occams::Cms::Layout.new(content: "c {{cms:text content}} d")
    layout_b.parent = layout_a
    expected = [
  ...
  def test_app_layouts_for_select
    FileUtils.touch(File.expand_path("app/views/layouts/occams/admin/cms/nested.html.erb", Rails.root))
    FileUtils.touch(File.expand_path("app/views/layouts/occams/_partial.html.erb", Rails.root))
    FileUtils.touch(File.expand_path("app/views/layouts/occams/not_a_layout.erb", Rails.root))
    ...
    FileUtils.rm(File.expand_path("app/views/layouts/occams/not_a_layout.erb", Rails.root))
  end
  ...
  def test_multiple_view_paths
    ...
    assert_equal ["additional_layout", "occams/admin/cms"], Occams::Cms::Layout.app_layouts_for_select(view_paths)
  ensure
