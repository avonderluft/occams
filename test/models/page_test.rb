...
class CmsPageTest < ActiveSupport::TestCase
  ...
  def test_fixtures_validity
    Occams::Cms::Page.all.each do |page|
  ...
  def test_validation_of_parent_relationship
    ...
    @page.parent = occams_cms_pages(:child)
    assert @page.invalid?
    assert_has_errors_on @page, :parent_id
  end
  ...
  def test_create
    page_count      = -> { Occams::Cms::Page.count }
    fragment_count  = -> { Occams::Cms::Fragment.count }
  ...
  def test_update_with_file_removal
    id = occams_cms_fragments(:file).attachments.first.id
  ...
  def test_initialization_of_full_path
    ...
    page = Occams::Cms::Page.new(new_params)
    assert page.invalid?
    ...
    page = @site.pages.new(new_params(parent: occams_cms_pages(:child)))
    assert page.valid?
  ...
  def test_cascading_destroy
    assert_difference(-> { Occams::Cms::Page.count }, -2) do
      assert_difference(-> { Occams::Cms::Fragment.count }, -5) do
  ...
  def test_options_for_select
    ...
    assert_equal options, Occams::Cms::Page.options_for_select(site: @site).map(&:first)
    ...
    assert_equal [], Occams::Cms::Page.options_for_select(site: @site).map(&:first)
  end
  ...
  def test_scope_published
    assert_equal 2, Occams::Cms::Page.published.count
    occams_cms_pages(:child).update_columns(is_published: false)
    assert_equal 1, Occams::Cms::Page.published.count
  end

  def test_root?
    assert @page.root?
    refute occams_cms_pages(:child).root?
  end

  def test_url
    assert_equal '//www.example.com/', @page.url
    assert_equal '//www.example.com/child-page', occams_cms_pages(:child).url

    assert_equal '/', @page.url(relative: true)
    assert_equal '/child-page', occams_cms_pages(:child).url(relative: true)

    @site.update_columns(path: '/en/site')
    @page.reload
    occams_cms_pages(:child).reload

    assert_equal '//www.example.com/en/site/', @page.url
    assert_equal '//www.example.com/en/site/child-page', occams_cms_pages(:child).url

    assert_equal '/en/site/', @page.url(relative: true)
    assert_equal '/en/site/child-page', occams_cms_pages(:child).url(relative: true)
  end

  def test_url_with_public_cms_path
    Occams.config.public_cms_path = '/custom'
    assert_equal '//www.example.com/custom/', @page.url
    assert_equal '//www.example.com/custom/child-page', occams_cms_pages(:child).url

    assert_equal '/custom/', @page.url(relative: true)
    assert_equal '/custom/child-page', occams_cms_pages(:child).url(relative: true)
  end
  ...
  def test_identifier
    ...
    assert_equal 'child-page',  occams_cms_pages(:child).identifier

    @page.update_column(:slug, 'index')
    assert_equal 'index', occams_cms_pages(:default).identifier
  end
  ...
  def test_translate
    ...
    translation.update_columns(layout_id: occams_cms_layouts(:nested).id)

    @page.translate!
    assert @page.readonly?

    assert_equal occams_cms_layouts(:nested), @page.layout
  ...
  def test_translate_with_no_translations
    I18n.locale = :fr
    Occams::Cms::Translation.delete_all
