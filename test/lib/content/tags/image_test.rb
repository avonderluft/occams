...
class ContentTagsImageTest < ActiveSupport::TestCase
  ...
  def test_init
    tag = Occams::Content::Tags::Image.new(context: @page, params: ['my_image'])
  ...
  def test_init_without_identifier
    ...
    assert_raises Occams::Content::Tag::Error, message do
      Occams::Content::Tags::Image.new(context: @page)
    end
  end

  def test_file
    tag = Occams::Content::Tags::Image.new(context: @page, params: [@file.label])
    assert_instance_of Occams::Cms::File, tag.file_record

    tag = Occams::Content::Tags::Image.new(context: @page, params: ['invalid'])
    assert_nil tag.file_record
  end

  def test_content
    tag = Occams::Content::Tags::Image.new(
      context: @page,
