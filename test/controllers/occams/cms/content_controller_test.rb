...
class Occams::Cms::ContentControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_show_with_app_layout
    @layout.update_columns(app_layout: "occams/admin/cms")
    get occams_cms_render_page_path(cms_path: "")
    assert_response :success
    assert assigns(:cms_page)
    assert_select "body.c-occams-cms-content.a-show"
  end

  def test_show_with_xhr
    @layout.update_columns(app_layout: "cms_admin")
    get occams_cms_render_page_path(cms_path: ""), xhr: true
    assert_response :success
    assert assigns(:cms_page)
    assert_no_select "body.c-occams-cms-content.a-show"
  end
  ...
  def test_show_with_redirect
    occams_cms_pages(:child).update_columns(target_page_id: @page.id)
    assert_equal @page, occams_cms_pages(:child).target_page
    get occams_cms_render_page_path(cms_path: "child-page")
  ...
  def test_show_with_redirect_and_site_path
    ...
    occams_cms_pages(:child).update_columns(target_page_id: @page.id)
    assert_equal @page, occams_cms_pages(:child).target_page
    get occams_cms_render_page_path(cms_path: "/test-site-path/child-page")
    assert_response :redirect
