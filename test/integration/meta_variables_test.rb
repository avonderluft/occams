...
class MetaVariablesIntegrationTest < ActionDispatch::IntegrationTest
  ...
  def test_redactor_js_variables
    ...
    assert_select "meta[name='cms-file-upload-path'][content='#{occams_admin_cms_site_files_path(site)}']"
    assert_select "meta[name='cms-pages-path'][content='#{occams_admin_cms_site_pages_path(site)}']"
