...
class RenderCmsIntergrationTest < ActionDispatch::IntegrationTest
  ...
  def setup
    ...
    occams_cms_layouts(:default).update_columns(content: "{{cms:text content}}")
    occams_cms_pages(:child).update(fragments_attributes: [
  ...
  def test_implicit_cms_page_with_site_path
    occams_cms_sites(:default).update_column(:path, "site-path")
    occams_cms_pages(:child).update(slug: "render-page")
  ...
  def test_cms_layout_defaults
    ...
    assert assigns(:cms_site)
    assert assigns(:cms_layout)
    assert_equal occams_cms_layouts(:default), assigns(:cms_layout)
  end

  def test_cms_layout
    content = "{{cms:text content}} {{cms:text content_b}} {{cms:text content_c}}"
    occams_cms_layouts(:default).update_columns(content: content)
